...
namespace BolosDoJacquinWeb.API.Services
{
    public class CloudinaryService : ICloudinaryService
    {
        ...
        {
            // Abre um fluxo de leitura de arquivo
            // using: garante que o string será fechado após o uso(libera a memória mesmo se der erro)
            using var stream = arquivo.OpenReadStream();

            // Monta os parâmetros do upload
            var uploadParams = new ImageUploadParams
            {
                // O arquivo em si: bine original + o fluxo de bytes a enviar
